...
bg = pygame.image.load('cesta2.jpg')
blue_car = pygame.image.load('pc.png')
yellow_car = pygame.image.load('yc.png')
green_car = pygame.image.load('zc.png')
white_car = pygame.image.load('bc.png')
...
while runtime:
    ...
    for event in pygame.event.get():
        if event.type==pygame.QUIT:
            runtime=False
    #puko se
    if(a-17-14<=x <= a+17+14):
        if(a1-38-33 <= y <= a1+39+34):
            runtime = False
    if(b-13-14 <= x <= b+14+14):
        if(b1-30-33 <= y <= b1+31+34):
            runtime = False
    if(c-12-14 <= x <= c+12+14):
        if(c1-29-33 <= y <= c1+29+34):
            runtime = False
